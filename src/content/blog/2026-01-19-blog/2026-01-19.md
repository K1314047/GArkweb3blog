...
我就会想起，当我在设置自定义节点的时候，我发现 Cloudflare 控制台的「对象存储页面」有一个 S3 API，在「特定存储桶的设置」里还有一个 S3 API。他们的差异就是是否含有存储桶名称。我在填写 S3 API 的时候多填写了 blog 路径，导致上传的地址是在 blog 存储桶的 blog 文件夹下，所以 Cloudflare 给出的路径就多了一个 blog 目录
